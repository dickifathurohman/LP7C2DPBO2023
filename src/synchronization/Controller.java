    ...
    public synchronized void keyReleased(KeyEvent e)
    {
        ...
        if(game.isRunning())
        {
            ...
            if((key == KeyEvent.VK_W) || (key == KeyEvent.VK_UP))
            {
                ...
                //Cek jika pergerakan sebelumnya sama dengan pergerakan sekrang atau tidak
                if(!game.getMove().equals("UP")){
                    game.setScore(+1);
                    game.setMove("UP"); //mengset prev move dengan pergerakan yg sekarang
                }   
            }
            if((key == KeyEvent.VK_A) || (key == KeyEvent.VK_LEFT))
            {
